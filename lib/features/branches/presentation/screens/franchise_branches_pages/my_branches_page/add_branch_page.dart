...
class _AddBranchPageState extends State<AddBranchPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final userId = isFranchisee ? widget.arguments['userId'] as String : null;
    final franchise = isFranchisee ? null : widget.arguments['franchise'] as Franchise;

    return Scaffold(
      appBar: AppBar(title: Text(isFranchisee ? 'Создать заявку на филиал' : 'Добавить филиал')),
      body: BlocProvider(
                      ...
                      if (_formKey.currentState!.validate()) {
                        if (isFranchisee) {
                          context.read<BranchBloc>().add(CreatePendingBranch(
                            franchiseId: widget.arguments['franchiseId'] ?? 'test_franchise_id',
                            ownerId: widget.arguments['franchiseOwnerId'] ?? 'test_franchise_owner_id',
                            requesterId: userId!,
                          ...
                          final branch = FranchiseBranch(
                            ...
                            ownerId: userId ?? sl<FirebaseAuth>().currentUser!.uid,
                            name: _nameController.text,
                            location: _locationController.text,
                            royaltyPercent: double.parse(_royaltyPercentController.text),
                            workingHours: _workingHoursController.text,
